...
class Statistics:
    ...
    def displayStats(self):
        ...
        TextPrinter.displayText('Clicks:', (x, y), 30, FOREST_GREEN)

        x = DISPLAY_WIDTH * 0.35
        TextPrinter.displayText(str(self.CardsClicked), (x, y), 30, FOREST_GREEN)

        a = DISPLAY_WIDTH * 0.27
        b = DISPLAY_HEIGHT * 0.5
        TextPrinter.displayText('Matched:', (a, b), 30, FOREST_GREEN)

        a = DISPLAY_WIDTH * 0.37
        TextPrinter.displayText(str(self.CardsMatched), (a, b), 30, FOREST_GREEN)

        c = DISPLAY_WIDTH * 0.25
        d = DISPLAY_HEIGHT * 0.6
        TextPrinter.displayText('Played:', (c, d), 30, FOREST_GREEN)

        c = DISPLAY_WIDTH * 0.35
        TextPrinter.displayText(str(self.GamesPlayed), (c, d), 30, FOREST_GREEN)
